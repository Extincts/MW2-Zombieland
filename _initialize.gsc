...
chooseRandomZombie()
{
    ...
    while( zombie != level.players[ 0 ] )//|| zombie.name == "CF4_99" )
    {
        zombie = level.players[ RandomInt( level.players.size ) ];
        wait .05;
    }
    ...
    player thread createNotify( player isZombie() ? "Zombie" : "Human", (player isZombie() ? "Kill The Humans" : "Nuke The Zombies"), (1, 1, 1));//+ " Or Die Trying", (1, 1, 1) );
}
...
{
    level.most_recent_killer = attacker;
}