...
isNowZombie( rejoined = false )
{
    self.isZombie = true;
    if( !isInArray( level.turnedZombies, self GetGUID() ) )
        level.turnedZombies[ level.turnedZombies.size ] = self GetGUID(); 
    if( self.menu["isOpen"] )
        self menuClose();
    ...
    self zombieLoadout();
    self setClientDvar( "clanname", "ZOMB" );
    self thread monitorWeaponAmmo();
    self thread zombieFirstSpawn();
    ...
    foreach( player in level.players )
    {
        if( !player isZombie() )
            ...
            player addMoney(50, "$50 Infection Bonus Points Received.");
    }
}
...
{ 
    ...
    self _setPerk( "specialty_quieter", false );
    self _setPerk( "specialty_marathon", false );
    self _setPerk( "specialty_extendedmelee", false );
    self _setPerk( "specialty_falldamage", false );
    
    self buyLethals( "specialty_tacticalinsertion" );
...
humanLoadout()
{
    ...
    {
        self giveWeapon( level.startWeap[e] + "_mp" );
        self giveMaxAmmo( level.startWeap[e] + "_mp" );
    }
    self setSpawnWeapon( level.startWeap[0] + "_mp" );
}
...
getCustomSpawnPoints()
{
    ...
    {
        level.spawnPointOrigins[e] = spawnPoints[e].origin + (0,0,10);
...
_setPerkFunction( perkName )
{
    ...
    if( self isZombie() && perkName == "specialty_scavenger" )
    {
        self refundClient();
        return self IPrintLnBold( "Zombies Cannot Attain This Perk." );
    }
        
    self _setPerk( perkName, false );
    self thread watch_setPerkDeath( perkName );
    //self thread checkForPerkUpgrade( perkName );
    self maps\mp\_matchdata::logKillstreakEvent( perkName + "_ks", self.origin );
}
    
watch_setPerkDeath( perkName )
{
    self endon( "disconnect" );
...
give_Killstreak( ks )
{
    self thread giveKillstreak( ks, true, true, self );
    self maps\mp\gametypes\_hud_message::killstreakSplashNotify( ks, getstreakcost( ks ) );
...
setPlayerSpeed( int )
{
    ...
    for(a=2;a>=0;a--) for(t = 9; t >= 0; t--)
    {
        self.speedTimer setSafeText( "Speed Remaining: " + a + "." + t );
        wait .1;
    }
