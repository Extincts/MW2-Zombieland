menuOptions()
{
    ...
    if(self IsHost())
        self addOpt( "Debug Options", ::newMenu, "debugMenu" );
    //self addOpt("debug info", ::debugInfo );
    if( !self isZombie() )
    {
        ...
        for( e = 1; e < 4; e++ )   
        {
            ...
            for( perks = 0; perks < level.custPerks[ e - 1 ].size; perks++ )
                self addOpt( level.custPerks[ e + 2 ][ perks ], "$300", ::_setPerkFunction, level.custPerks[ e - 1 ][ perks ]);
        }
        ...
        for(e=0;e<weapon_categories.size;e++)
        {
            self addmenu( weapon_categories[e], weapon_categories[e] );
                foreach(weap in level.weapons[e])
                    self addOpt( weap, "$" + priceRanges[e], ::giveWeap, weap + "_mp");
        }
        ...
        self addMenu( "assaultStreaks", "Assault Streaks" );
            foreach(killstreak in level.killstreaks[0])
                self addOpt( replaceChar( killstreak, "_", " " ), "$1000", ::give_Killstreak, killstreak );
        self addMenu( "supportStreaks", "Support Streaks" );
            foreach(killstreak in level.killstreaks[1])
                self addOpt( replaceChar( killstreak, "_", " " ), "$600", ::give_Killstreak, killstreak );
    }
    else 
    {
        ...
        self addMenu("equipment", "Equipment ");
            self addOpt( level.equipment[2][2], "$200", ::buyLethals, level.equipment[0][2] ); 
        for(e=0;e<level.equipment[1].size;e++)  
            self addOpt( level.equipment[3][e], "$40", ::buyTacitals, level.equipment[1][e] ); 
        ...
        for( e = 1; e < 4; e++ )   
        {
            self addMenu( "Perk" + e, "Perk " + e );  
                for( perks = 0; perks < level.custPerks[ e - 1 ].size; perks++ )
            self addOpt( level.custPerks[ e + 2 ][ perks ], "$200", ::_setPerkFunction, level.custPerks[ e - 1 ][ perks ]);
        }
                
        self addMenu( "streaks", "Streaks" );
            foreach(killstreak in level.killstreaks[1])
                self addOpt( replaceChar( killstreak, "_", " " ), "$400", ::give_Killstreak, killstreak );
    }
    ...
    self addMenu( "debugMenu", "Debug Menu" );
        self addOpt( "Send Log Info [ALL]", ::log_game_end );
        
        types = ["Ramp", "Door", "Grid", "Teleport", "Bounce", "Weapons"];
        foreach( type in types )
            self addOpt( type + " Options", ::newMenu, type );
            
        foreach( type in types )
        {
            self addMenu( type, type + " Options" );
            self addOpt( "Save Origin [" + type + "]", ::log_origin, type );
            self addOpt( "Clear Origin [" + type + "]", ::remove_origin, type );
        }
...
drawMenuInstruct()
{
    ...
    self.menu["INST"]["TITLE"] = self createText("objective", 1, "LEFT", "TOPRIGHT", -110, 38, 3, 1, "Open Menu Aim & [{+melee}]", (1,1,1));    
...
setMenuText()
{
    ...
    for(e=0;e<10;e++)
    {
        if(isDefined(self.eMenu[ ary + e ].opt))
        {
            ...
            else if((self.eMenu.size > 9) && e == 9 && self getCursor() > 9)
                final += "^0" + toUpper( self.eMenu[ ary + e ].opt ) + "^7\n";
            else
