...
smallNotify( text )
{ 
    if( IsDefined( self.notify ) )
    {
        self.notify destroy();
        self notify("stop_smallNotify");
    }   
    
    self endon("stop_smallNotify");
...
gameHuds()
{
    if(isDefined(self.playerHuds))
        return;
    self.playerHuds = [];
    self.playerHuds[ "title" ] = createText("objective", 1, "LEFT", "TOPRIGHT", -110, -12, 3, 1, (self isZombie() ? "Zombie" : "Human"), (1, 1, 1));// + ": Health & Shield", (1, 1, 1));  
...
removeChars( string, remove )
{
    ...
    for(e=0;e<string.size;e++)
    {
        ...
        upcoming = "";
        for(a=0;a<3;a++)
            upcoming += string[((e-1) + a) + remove.size];
